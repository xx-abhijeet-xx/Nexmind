...
const MSGS = [
  ...
  { role:'ai', stream:'I can help with almost anything:\n\nâ€¢ **Research** â€” search the web live for current info\nâ€¢ **Documents** â€” analyze PDFs and images\nâ€¢ **Writing** â€” drafts, summaries, translations\nâ€¢ **Code** â€” debug, explain, generate\n\nWhat would you like to start with?' },
...
export default function ChatDemo() {
        ...
        for (let m of MSGS) {
          ...
          if (m.role === 'user') {
            ...
            for (let i = 0; i < m.stream.length; i++) {
              if(!active) break;
              rendered += m.stream[i];
              setMessages(prev => prev.map(msg => msg.id === msgId ? { ...msg, content: rendered, cursor: true } : msg));
              if(chatRef.current) chatRef.current.scrollTop = chatRef.current.scrollHeight;
              await sleep(m.stream[i] === '\n' ? 60 : 14);
            }
            if(!active) break;
            setMessages(prev => prev.map(msg => msg.id === msgId ? { ...msg, cursor: false } : msg));
            await sleep(1400);
          }
        }
        await sleep(2200);
  ...
  const formatText = (txt) => {
    const bolded = txt.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br/>');
    ...
    <div className="hero-right">
      <div className="chat-window">
        <div className="chat-titlebar">
          ...
          <div className="model-pill"><div className="mpulse"></div>Gemini 1.5 Pro</div>
        </div>
        <div className="chat-msgs" ref={chatRef}>
          {messages.map(msg => (
            <div key={msg.id} className={`msg ${msg.role}`}>
              <div className="mavatar">{msg.role === 'ai' ? 'P' : 'U'}</div>
        ...
        <div className="attach-strip" style={{ display: attachments ? 'flex' : 'none' }}>
          <div className="abadge">ðŸ“„ report.pdf</div>
          <div className="abadge">ðŸŒ web search</div>
        </div>
        
        <div className="chat-bar">
          <div className="fake-input">{fakeInput}</div>
          <button className="send-btn" aria-label="Send">
            <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="#0c0c0e" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 8h10M9 4l4 4-4 4"/></svg>
          </button>
        </div>
      </div>
    </div>
